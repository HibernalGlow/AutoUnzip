...
python -m findz "ext = 'jxl' AND archive RLIKE '\\.zip$'" "E:\1Hub\EH\[02COS]" -A -o .\jxl_results.txt
...
| `width`      | 图片宽度           | `width > 1200`             |
| `height`     | 图片高度           | `height > 630`             |
| `resolution` | 分辨率             | `resolution = '1920x1080'` |
| `megapixels` | 百万像素           | `megapixels > 2`           |
...
## 图片过滤 (Image Filtering)

此功能需要使用 `-I` 或 `--with-image-meta` 选项启用。它会读取图片文件的宽高、分辨率、百万像素等信息。
